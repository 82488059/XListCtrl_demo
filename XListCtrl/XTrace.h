...
public:
	...
	void operator() (LPCTSTR lpszFormat, ...)
	{
		...
		{
#ifdef _UNICODE
			if (_snwprintf_s(buf1, BUFFER_SIZE - 1, _T("%s(%d) : [%X] %s"),
						cp, m_line, GetCurrentThreadId(), lpszFormat) < 0)
#else
			if (_snprintf_s(buf1, BUFFER_SIZE - 1, _T("%s(%d) : [%X] %s"),
				cp, m_line, GetCurrentThreadId(), lpszFormat) < 0)
#endif
		...
		{
#ifdef _UNICODE
			if (_snwprintf_s(buf1, BUFFER_SIZE - 1, _T("%s(%d) : %s"),
						cp, m_line, lpszFormat) < 0)
#else
			if (_snprintf_s(buf1, BUFFER_SIZE - 1, _T("%s(%d) : %s"),
				cp, m_line, lpszFormat) < 0)
#endif
				buf1[BUFFER_SIZE-1] = _T('\0');
		}
		...
		if (_vsntprintf_s(buf2, BUFFER_SIZE-1, buf1, va) < 0)
		...
		if (XTRACE_FILE)
		{
			...
			if (cp != NULL)
				*(cp+1) = _T('\0');
			_tcscat_s(szPathName, _T("\\_trace.log"));
...
public:
	...
	~xtracing_entry_output_debug_string()
	{
		...
		_stprintf_s(buf3, _T("======  exiting scope:  %s"), buf2);
	...
	void operator() (LPCTSTR lpszFormat, ...)
	{
		...
		if (XTRACE_SHOW_THREAD_ID)
		{
			if (_sntprintf_s(buf1, BUFFER_SIZE-1, _T("%s(%d) : [%X] ======  %s"), 
						cp, m_line, GetCurrentThreadId(), lpszFormat) < 0)
				buf1[BUFFER_SIZE-1] = _T('\0');
		}
		else
		{
			if (_sntprintf_s(buf1, BUFFER_SIZE - 1, _T("%s(%d) : ======  %s"),
						cp, m_line, lpszFormat) < 0)
				buf1[BUFFER_SIZE-1] = _T('\0');
		}
